...
def std_norm(X):
    ...
    There is similar scaler in SciKit Learn called StandardScaler

    Input Arguments:
    X - Matrix or m by 1 vector for single feature 
    Return:
    X_norm - Scaled/Normalized form of X
    avg - The mean that is used to calculate the Z-score
    std - The standard deviation taht is used to calculate the Z-score
    """
    ### the following check if data type is Series
...
def reverse_bw_std_norm(coef, intercept, avg, stddev):
    ...
    Input Arguments:
    coef - The coeficient or weights from machine learning training
    intercept - The Y-intercept from machine learning training
    avg - The mean that is used to calculate the scaled data
    stddev - The standard deviation that is used to calculate the scaled data

    Return:
    or_coef_gd - The coeficient or weights that can use to calculate prediction without need to scaled the query
    or_intercept_gd - The intercept that can use to calculate prediction without need to scaled the query
    '''
    n = coef.shape[0]
